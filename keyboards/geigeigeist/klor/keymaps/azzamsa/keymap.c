...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_BASE] = LAYOUT(
        ...
        XXX, KC_Z,  KC_X,  KC_C,    KC_D,    KC_V,    ___,      ___,     KC_K,    KC_H,     KC_COMM, KC_DOT, KC_SLSH, XXX,
                           XXX,     KC_ESC,  KC_SPC,  KC_TAB,   KC_ENT,  KC_BSPC,  KC_DEL,  XXX
    ),
    ...
    [_NAV] = LAYOUT(
        ...
        XXX, XXX,     KC_BSPC,  KC_ENT,  KC_DEL,  XXX, ___,  ___,     KC_INS,   KC_HOME, KC_PGDN, KC_PGUP, KC_END,  XXX,
...
smtd_resolution on_smtd_action(uint16_t keycode, smtd_action action, uint8_t tap_count) {
    switch (keycode) {
        ...
        SMTD_LT(KC_ESC, _MEDIA)
        SMTD_LT(KC_SPACE, _NAV)
        SMTD_LT(KC_TAB, _MOUSE)

        // righ side
        SMTD_MT(KC_N, KC_LSFT)
        SMTD_MT(KC_E, KC_LCTL)
        SMTD_MT(KC_I, KC_LALT)
        SMTD_MT(KC_O, KC_LGUI)

        SMTD_LT(KC_ENT, _SYM)
        SMTD_LT(KC_BSPC, _NUM)
        SMTD_LT(KC_DEL, _FUNC)
    }
